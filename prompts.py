...
- Call get_weather for the destination coordinates (or representative point) to assess conditions that might affect the animal during transport.
...
- Prefer PEDESTRIAN when:
  - The straight‑line or walking distance between origin and destination is very small (for example, neighboring buildings, same block, a few hundred meters),
...
- Consider BICYCLE when:
  - Distance is short to moderate for a bike (for example, a few kilometers),
...
YOUR GOAL: Provide clean, structured, realistic transport guidance that balances safety, practicality, and efficiency, rather than always choosing the most conservative heavy vehicle.
""".strip()
